...
## 4. 노트북에서 본인 데이터로 분석하기 (권장)

### 4.1 빠른 시작 (설정 셀만 수정)

`04_Notebooks/nonparametric_analysis_template.ipynb`을 열고 **설정 셀 하나만 수정**하면 됩니다:
...
FILE_NAME       = "my_data.csv"       # 02_Data/ 폴더에 넣은 CSV 파일명
TARGET_COLUMN   = "매출액"             # 분석할 숫자 컬럼

# 두 그룹 비교 (선택) - 안 하면 빈 문자열로
GROUP_COLUMN    = "지역"
GROUP_A_VALUE   = "서울"
GROUP_B_VALUE   = "부산"

# 전/후 비교 (선택) - 안 하면 빈 문자열로
BEFORE_COLUMN   = ""
AFTER_COLUMN    = ""

# 다중 그룹 비교 (선택) - 안 하면 빈 리스트로
MULTI_GROUP_VALUES = []

# 반복 측정 비교 (선택) - 안 하면 빈 리스트로
REPEATED_COLUMNS   = []

# 상관분석 컬럼 (비워두면 모든 숫자 컬럼 자동 사용)
CORRELATION_COLUMNS = []
```

- 데이터 검증 셀이 설정값을 자동으로 확인하고, 오류 시 한국어 안내를 출력합니다
- 미설정 분석(그룹비교, 전/후 비교 등)은 에러 없이 자동으로 건너뜁니다
- 데이터 형식 안내: `02_Data/데이터_준비_가이드.md` 참고

### 4.2 개별 함수 직접 호출

Python 코드에서 개별 함수를 직접 호출할 수도 있습니다:
